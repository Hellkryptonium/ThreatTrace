    ...
    <main className={`${styles.shell} ${styles.investigation}`}>
      ...
      <section className={styles.reportWorkspace}>
        <div className={styles.workspaceScroll}>
          ...
          {openSection === "authentication" && <div className={styles.workspaceView}><section className={styles.workspaceCard}><h2>Authentication</h2>{Object.entries(analysis.authentication).map(([key, value]) => <p className={styles.auth} key={key}><span>{key.toUpperCase()}</span><strong>{value ?? "NOT FOUND"}</strong></p>)}</section></div>}
          ...
          {openSection === "infrastructure" && <div className={styles.workspaceView}><section className={styles.workspaceCard}><h2>Infrastructure enrichment</h2>{enrichment ? <><p className={styles.enrichmentNote}>Best-effort public lookups. Results are contextual evidence, not attribution.</p>{enrichment.ips.map((item) => <div className={styles.enrichmentBlock} key={item.ip}><strong>{item.ip}</strong><span>{[item.city, item.region, item.country].filter(Boolean).join(", ") || "Approximate network location unavailable"}</span><span>{item.isp || item.organization || "Network unavailable"}{item.asn ? ` · ${item.asn}` : ""}</span></div>)}{enrichment.domains.map((item) => <div className={styles.enrichmentBlock} key={item.domain}><strong>{item.domain}</strong><span>DNS: {item.dns.addresses.join(", ") || "No A record"}</span><span>MX: {item.dns.mx.join(", ") || "No MX record"}</span><span>Registrar: {item.rdap?.registrar || "Unavailable"}</span></div>)}{enrichment.urls.map((item) => <div className={styles.enrichmentBlock} key={`${item.source}-${item.url}`}><strong>{item.source}</strong><span>{item.verdict || `Malicious: ${item.malicious ?? 0} · Suspicious: ${item.suspicious ?? 0}`}</span></div>)}</> : <p className="muted">Enrichment was not available for this analysis.</p>}</section></div>}
