...
export default function UploadPage() {
  ...
  return (
    <main className="shell">
      <header className="topbar"><span className="eyebrow">THREATTRACE <small>/ INTAKE</small></span><span className="top-links"><a href="/emails">Gmail inbox</a><div className="dropdown-container" ref={dropdownRef}><button className="dropdown-toggle" onClick={() => setDropdownOpen(!dropdownOpen)}>{user?.name ?? "Checking session..."} ▾</button><div className={`dropdown-menu ${dropdownOpen ? 'open' : ''}`}><button onClick={handleLogout} className="dropdown-item">Logout</button></div></div></span></header>
